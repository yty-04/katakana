# カタカナ ひょう（子供のカタカナ学習アプリ）

- 本体は `index.html` の1ファイル（HTML/CSS/JavaScript。ライブラリなし）。ほかにホーム画面用アイコン `icon-192.png` `icon-512.png` と `manifest.webmanifest`
- 公開ページ: https://yty-04.github.io/katakana/ （GitHub Pages・main ブランチの内容がそのまま公開される）
- 使う人: 子供（Android の Chrome でホーム画面から開く）。直す人: 保護者（スマホの Claude Code と PC の Claude Code の両方から）
- 返答の相手は保護者（大人）。ふつうの大人向けの日本語（漢字まじり・です・ます）で書く。ひらがなばかりの子供向けの文にはしない。専門用語は避けるか、ひとこと説明を添える
- 画面に出る文字（アプリの中の言葉）は子供向けのひらがな・カタカナのままでよい

## 機能（直すときに壊さないこと）
- 📖 アイウエオ: 50音表（ぜんぶ／おおきく）。押すと読み上げ
- 🚂 ゲーム: もんだい／れんしゅう。5問正解でお祝い（電車）→スタンプ ゲット→メニュー。ゲーム中はオルゴール風 BGM（手作り・お祝い中は止まる）
- 🎫 スタンプ: 20個でカード完成（15%でキラキラ）。印刷・画像保存。「○こ 🔍」を押すと全スタンプを番号つきで一覧。📅 がんばりカレンダー（1問できた日に🌟・つづけた日数。その日の1問めができたら自動でカレンダーへ移動→今日のマスを押して16種類から好きなシールを貼る（ポン！）→ゲームに戻る。あとから貼り替えも可）
- ⚙️ 大人の方へ（右上の歯車）: 使い方の説明、お子さんの名前（あいさつ・お祝いで呼びかけ）、ゲームのレベル（自動＝スタンプ数 0/10/30/60 で1〜4、または手動1〜5。5は「ー・ッ・ャュョ」入りの言葉 WORDS_SP で手動のみ）、まちがえやすい字と5問ごとのまちがい回数の記録、こえ／おんがく の音量、スタンプを押す／減らす／全部消す、引っ越しコード（KTKN1:）
- 記録はブラウザの localStorage（katakana_stamps / katakana_cards / katakana_slots、音量は katakana_volume、レベル設定は katakana_level、名前は katakana_name、まちがい記録は katakana_stats / katakana_history、カレンダーは katakana_days / katakana_stickers）。**キー名とコード形式は変えない**（子供の記録が消える）
- つかう人（3人）: ⚙️ で 1ばん／2ばん／ゲスト を切り替え（katakana_profile）。1ばんは上のキー名そのまま、2ばん・3ばんは各キーの後ろに `_p2` / `_p3` を付けて別保存（`PK()` を通す）。音量だけは共通

## 声
- speechSynthesis を使用。`voiceScore()` で自然な声を優先（Natural/Nanami > Google > Kyoko拡張 > その他 > 旧Microsoft）
- LINE やアプリ内ブラウザ（WebView）では手作りの合成音声 `speakSynth()` に切り替わる（機械っぽい）
- 残りの課題: 声の高さ・速さの調整、声と動きのずれ（今は別々のタイマーで決め打ち。声の onstart/onend に動きを合わせるのが直し方）

## 直すときのルール
- 頼まれた所だけ直す。関係ない所を書き直さない
- 直したら、変えた所と「どう試せばいいか」を短く伝える
- 公開ページへの反映は main への反映が必要。保護者が「反映して」と言ったら main に入れる

## PCのフォルダで作業するとき（ローカルのセッション）
- 作業の前に必ず `git pull` で最新を取る（スマホのクラウドセッションで直した分を取り込むため）
- 「反映して」と言われたら、main にコミットして `git push` する（1〜2分で公開ページに反映）
